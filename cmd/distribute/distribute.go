	...
	for _, module := range []string{"bls", "snark"} {
		...
		for pkg, platformsForPkg := range platforms {
			...
			for _, platform := range platformsForPkg {
				...
				// Formats the source. As of writing (24 Jan 2022) this will add a Go 1.17 format build directive to complement
				// the older format ones generated by this generator. This is not currently required, but may become so in the future.
				// See https://go.googlesource.com/proposal/+/master/design/draft-gobuild.md#transition
				fmtModulePlatform, err := format.Source([]byte(modulePlatform))
				panicIfError(err)

				err = writeFile(modulePlatformPath, fmtModulePlatform)
				panicIfError(err)
